...
性能問題では、`遅い` を 1 つの現象として扱わないことが重要です。CPU、memory、network、DB、アプリ実装、resource 制限のどれが律速かを仮説で分け、平均値ではなく tail latency と偏りで見る癖を付けられると、この回は実務的になります。
...
平均 latency が安定していても、一部の Pod だけ遅い、一部のリクエストだけ詰まる、といった問題は簡単に隠れます。そのため p95 や p99、Pod ごとの偏りを見ることが重要です。負荷試験は `重かった` を確認する場ではなく、どの層が限界に達しているかを切り分ける場だと理解してください。

また Kubernetes では CPU throttling や requests / limits の設定そのものが性能問題を生むことがあります。アプリコードだけを疑うのでなく、プラットフォーム設定まで含めて見ることが実務で強い視点です。
