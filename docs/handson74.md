...
外部依存は `クラスタの外にあるから別問題` ではなく、サービス全体の failure mode の一部です。この回では timeout、retry、circuit breaker、bulkhead、egress 制御の役割差を説明し、外部障害で全部巻き込まない設計を考えられるようになることがポイントです。
...
この回の失敗は、外部依存を `接続先情報` としてしか見ないと起きやすいです。背景には `外部障害時の degrade 方針` と `障害増幅防止` を設計していないことがあります。

- retry を入れれば安全だと思う
- 外部依存一覧がなく failure mode を語れない
- degrade と完全停止の境界を決めない
