...
- `bash scripts/install-monitoring.sh`: Prometheus と Grafana を Helm で導入する
- `kubectl apply -k manifests/base/monitoring`: この教材用の monitoring manifest を反映する
- `kubectl apply -f manifests/base/user-service/servicemonitor.yaml`: user-service を Prometheus 監視対象に追加する
- `kubectl apply -f manifests/base/item-service/servicemonitor.yaml`: item-service を Prometheus 監視対象に追加する
- `kubectl get pods -n observability`: 監視系 Pod が起動しているか確認する
- `kubectl get servicemonitor -A`: 監視対象の定義が作成されたか確認する
- `kubectl get ingress -n observability`: Grafana 入口が作成されたか確認する

このコマンドで確認するのはここ:

- `kubectl get pods -n observability`: `READY`, `STATUS`, `RESTARTS` を見て Prometheus/Grafana 系が安定稼働しているか確認する
- `kubectl get servicemonitor -A`: apps と observability の監視対象が作成されているかを見る
- `kubectl get ingress -n observability`: `HOSTS` と `PORTS` を見て Grafana の入口があるか確認する
...
```bash
for i in {1..20}; do curl -s http://app.localtest.me/api/users > /dev/null; curl -s http://app.localtest.me/api/items > /dev/null; done
```

このコマンドの目的:

- API に軽くアクセスを流して、Grafana 上の数値変化を観察しやすくする

このコマンドで確認するのはここ:

- Grafana 側で CPU、Memory、request 数などの値が少し変化するかを見る
...
監視では `画面が開く` と `正しく収集できている` を分けて考える必要があります。Grafana は表示面、Prometheus は収集面、ServiceMonitor は収集対象定義なので、どの層で止まっているかを分けて見るのがコツです。
