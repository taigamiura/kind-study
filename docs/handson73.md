...
監査対応では、技術的に安全であることと、後から説明責任を果たせることを両立する必要があります。この回では audit trail、retention、data classification、承認記録を 1 本の証跡設計として説明できるようになることが重要です。
...
この回の失敗は、監査対応を `ログを残すこと` だけだと考えると起きやすいです。背景には `証跡`, `分類`, `保持`, `削除` を一連の運用として見ていないことがあります。

- 技術ログと監査証跡を同じものとして扱う
- retention を決めても削除方針を持たない
- data classification がないまま全データを同じように扱う
