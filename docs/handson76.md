...
非機能要件は `高性能にする話` だけではなく、業務要求に応じて性能、可用性、監査性、分離性、保守性の重みを変える話です。この回では、同じ Kubernetes 構成でも業種が変わると最適解が変わる理由を説明できるようになることが重要です。

## 確認するとしたらどこを見るか

- 非機能要件では性能、可用性、監査性、分離性、保守性のどれが重い業種かを見る
- 技術選定の理由が業務要求や契約要件につながっているか確認する
...
この回の失敗は、NFR を `性能要件の別名` として扱うと起きやすいです。背景には `業務要求ごとに重くなる非機能が違う` という前提の見落としがあります。

- 技術的に良い構成なら業種差を吸収できると思う
- 可用性だけ見て監査性や分離性を見落とす
- 高価な構成を入れれば安心だと考える
