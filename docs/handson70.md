...
observability 実装では、`集められること` と `安全に運用できること` を分けて考える必要があります。この回では collector、sampling、retention、PII マスキングを 1 本の pipeline 設計として説明できるようになることがゴールです。
...
この回の失敗は、可観測性を `全部集めておけば後で役立つ` という発想で設計すると起きやすいです。背景には `収集量`, `安全性`, `検索しやすさ`, `保持期間` を同時に設計していないことがあります。

- log と trace を何でも保存してコストを膨らませる
- token や PII の混入経路を洗い出さない
- collector を転送設定程度にしか見ない
