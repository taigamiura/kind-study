...
backup と restore はセットで初めて意味があります。この回では、保存できることより `どこまで戻せるか` と `どのくらい止まるか` を RPO/RTO 付きで説明できることを重視してください。

## 確認するとしたらどこを見るか

- backup runbook では保存先、世代、最新取得時刻、復元手順がセットで書かれているかを見る
- restore 観点では `どこまで戻るか` と `どれくらい止まるか` が説明できるか確認する
