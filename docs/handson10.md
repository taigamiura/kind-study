...
- `bash scripts/install-argocd.sh`: Argo CD 本体を Helm で導入する
- `kubectl apply -k manifests/overlays/local/gitops`: AppProject と Application 定義をまとめて反映する
- `kubectl get pods -n gitops`: Argo CD 関連 Pod が起動しているか確認する
- `kubectl get applications -n gitops`: 監視対象 Application が作成されたか確認する
- `kubectl get appprojects -n gitops`: Application を束ねる Project が作成されたか確認する
- `kubectl get ingress -n gitops`: Argo CD UI の入口が作成されたか確認する
- `kubectl get secret argocd-initial-admin-secret -n gitops -o jsonpath='{.data.password}' | base64 -d; echo`: 初期ログイン用 password を取得する
- `kubectl delete pod -l app.kubernetes.io/name=user-service -n apps`: Kubernetes の自己修復を観察するために Pod を意図的に消す
- `kubectl scale deployment web-app --replicas=3 -n apps`: Git とクラスタの drift を意図的に発生させる
- `kubectl get pods -n apps -w`: 変更後の Pod 変化を連続で観察する

このコマンドで確認するのはここ:

- `kubectl get pods -n gitops`: `STATUS` と `READY` を見て Argo CD 本体が起動しているか確認する
- `kubectl get applications -n gitops`: `SYNC STATUS` と `HEALTH STATUS` を見て、何が `Synced` / `OutOfSync` なのか確認する
- `kubectl get appprojects -n gitops`: AppProject が作成済みかを見る
- `kubectl get ingress -n gitops`: `HOSTS` に argocd.localtest.me が出ているかを見る
- `kubectl get secret argocd-initial-admin-secret -n gitops -o jsonpath='{.data.password}' | base64 -d; echo`: password を取り出せること自体を確認する
- `kubectl get pods -n apps -w`: Pod 削除後に自動で再作成されるか、scale 変更後に replica 数がどう変わるかを連続で見る
...
GitOps の本質は、`クラスタで変える` より `Git に正しい状態を書く` ことです。この回では、Argo CD が何を監視し、local apply がなぜ戻されるのか、Synced と Healthy が何を意味するのかを説明できればゴール達成です。
...
この回では、`Kubernetes が戻したのか` と `Argo CD が戻したのか` を混同しやすいです。Pod 削除は Deployment の自己修復、replica 変更は GitOps の差分修復という違いを意識すると理解しやすくなります。
