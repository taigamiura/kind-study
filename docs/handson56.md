...
multi-cluster は数を増やす話ではなく、何をどこまで分離し、どの障害でどこまで守るかを決める話です。environment 分離、blast radius、DR 対象を一緒に考えられるようになると、この回のゴールを満たせます。

## 確認するとしたらどこを見るか

- DR 設計では cluster 数ではなく、data、DNS、認証、証明書まで切り替え対象に入っているかを見る
- blast radius を考えるときは、1 つの障害で何が同時に止まるかを確認する
...
この回の失敗は、multi-cluster や DR を `増やせば安全` と考えると起きやすいです。箇条書きは別々に見えても、背景には `何の障害を想定し, どこまで戻したいか` の目的整理不足があります。
...
単一クラスタに全部を載せると運用は楽ですが、1 つの事故で全環境が止まる blast radius を持ち込みます。逆に cluster を増やしすぎると、権限、監視、証明書、運用負荷が一気に増えます。multi-cluster は増やすほどよいのでなく、何を守るために増やすかを説明できることが重要です。

また DR は compute の多重化だけでは成立しません。データ複製、DNS 切替、認証、証明書、外部接続まで含めて初めて `戻せる` 状態になります。
